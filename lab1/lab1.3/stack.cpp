...
Iterator Stack::iterator() const {
    return Iterator(_stack, _len);
}

void Stack::serialise(const std::string &path) const {
    std::ofstream out(path);
    if (out.is_open()) {
        out << _len << std::endl;
        for (int i = 0; i < _len; ++i)
            out << std::visit(CallToString{}, _stack[i]) << std::endl;
    }
    out.close();
}
